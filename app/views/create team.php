...
<head>
    ...
    <style>
        ...
        .content { flex: 1; padding: 24px 28px; display: flex; flex-direction: column; gap: 16px; overflow-y: auto; }
        .card { background: white; border-radius: 12px; border: 0.5px solid #ddd; padding: 22px 24px; width: 100%; }
        .card-title { font-size: 14px; font-weight: 500; color: #222831; margin-bottom: 4px; }
        .card-desc { font-size: 12px; color: #999; margin-bottom: 16px; padding-bottom: 14px; border-bottom: 0.5px solid #eee; line-height: 1.6; }
        ...
        .optional { color: #bbb; font-size: 11px; margin-left: 4px; }
        ...
        .photo-label { font-size: 13px; color: #00ADB5; font-weight: 500; }
        .photo-sub { font-size: 11px; color: #aaa; }
        ...
        .privacy-radio { width: 16px; height: 16px; border-radius: 50%; border: 2px solid #ccc; flex-shrink: 0; margin-top: 2px; display: flex; align-items: center; justify-content: center; }
        .privacy-opt.selected .privacy-radio { border-color: #00ADB5; }
        .privacy-radio-dot { width: 8px; height: 8px; border-radius: 50%; background: #00ADB5; display: none; }
        .privacy-opt.selected .privacy-radio-dot { display: block; }
        .privacy-opt-title { font-size: 13px; font-weight: 500; color: #222831; }
        .privacy-opt-sub { font-size: 11px; color: #aaa; margin-top: 3px; }
        ...
        .item-text { font-size: 13px; color: #222831; flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 500; }
        
        .section-dot { width: 8px; height: 8px; border-radius: 50%; background: #00ced1; flex-shrink: 0; }
        .member-avatar { width: 30px; height: 30px; border-radius: 50%; background: #1a1e2e; display: flex; align-items: center; justify-content: center; font-size: 11px; color: #00ced1; font-weight: bold; flex-shrink: 0; text-transform: uppercase; }
        
        /* X Button - FIXED VERTICAL ALIGNMENT */
        .btn-remove-item { 
            width: 24px; 
            height: 24px; 
            border-radius: 50%; 
        ...
        .btn-add { background: #00ADB5; color: white; border: none; border-radius: 8px; padding: 10px 18px; font-size: 12px; font-family: inherit; cursor: pointer; white-space: nowrap; font-weight: 500; transition: background 0.2s; }
        .btn-add:hover { background: #008c94; }
        .action-row { display: flex; gap: 12px; margin-top: 8px; }
        .btn-cancel { flex: 1; background: transparent; border: 1.5px solid #00ADB5; border-radius: 8px; padding: 13px; font-size: 13px; color: #00ADB5; cursor: pointer; font-family: inherit; font-weight: 500; transition: all 0.2s; }
        .btn-cancel:hover { background: #f0fdfd; }
        .btn-submit { flex: 1; background: #00ADB5; border: none; border-radius: 8px; padding: 13px; font-size: 13px; color: white; font-weight: 500; cursor: pointer; font-family: inherit; transition: background 0.2s; }
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="content">
            <div class="card">
                ...
                <div class="field-group">
                    ...
                    <textarea placeholder="Ceritakan sedikit tentang tim ini — tujuan, fokus, atau kegiatan yang dilakukan..." 
                              class="w-full bg-[#f7f7f7] border border-[#ddd] rounded-lg px-4 py-3 text-[13px] text-[#222831] focus:outline-none focus:border-[#00ADB5] focus:bg-[#f0fdfd] transition-all min-h-[100px] resize-none"></textarea>
                </div>
            </div>
                ...
                <div class="card flex flex-col">
                    ...
                    <div class="photo-upload flex-1" onclick="document.getElementById('team-photo-input').click()">
                        <div class="photo-icon">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#00ADB5" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                        </div>
            ...
            <div class="card">
                ...
                <div class="card-desc">Section adalah kategori konten di dalam tim. Daftar di bawah ini akan menjadi menu navigasi tim kamu.</div>
                
                <div class="dynamic-list" id="section-list">
                    <div class="list-item" data-value="Lomen">
                ...
                <div class="add-row">
                    <input type="text" id="sec-input" placeholder="Nama section baru..." maxlength="30" onkeypress="handleSectionInputKey(event)" 
                           class="flex-1 bg-[#f7f7f7] border border-[#ddd] rounded-lg px-4 py-2 text-[13px] text-[#222831] focus:outline-none focus:border-[#00ADB5] transition-all" />
...
<script>
    ...
    // 3. Update Sidebar Menu dari List Section
    ...
    function updateSidebarMenu() {
        ...
        sections.forEach((section, index) => {
            ...
            li.innerHTML = `
                ${activeIndicator}
                <span class="pl-8 uppercase tracking-wider text-sm ${textClass}">${sectionName}</span>
            `;
            sidebarMenu.appendChild(li);
        });
    }

    // 4. Tambah/Hapus Section
