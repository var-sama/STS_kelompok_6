...
<body class="bg-[#f6f7fb] flex h-screen font-sans overflow-hidden">
    ...
    <script>
    // --- 1. SINKRONISASI DATA (Sama persis dengan halaman Teams) ---
    const rawTeams = [
    ...
    const rawTeamsWithId = rawTeams.map((team, index) => ({ ...team, isRaw: true, rawId: index }));
    ...
    // --- 2. LOGIKA TAMPILKAN TIM BERDASARKAN KLIK (Ambil dari URL) ---
    const urlParams = new URLSearchParams(window.location.search);
    const teamId = urlParams.get('id');
    let currentTeam = null;

    if (teamId !== null) {
        currentTeam = teamsData.find(t => t.uiIndex === parseInt(teamId));
    }

    if (currentTeam) {
        // Ganti Nama dan Logo di Sidebar secara otomatis
        const teamNameEl = document.querySelector('h1.font-bold.text-lg');
        const teamLogoEl = document.querySelector('.w-32.h-32 img');

        if (teamNameEl) {
            teamNameEl.textContent = currentTeam.name;
            teamNameEl.title = currentTeam.name;
        }
        if (teamLogoEl) {
            teamLogoEl.src = currentTeam.img;
        }

        // --- TAMBAHAN BARU: Update Section Menu ---
        const sectionsContainer = document.getElementById('team-sections');
        
        // Ambil data section dari localStorage jika ada, 
        // Jika tidak ada (misal untuk tim default), gunakan nilai bawaan
        let teamSections = ['Lomen', 'Anggota', 'Proyek']; 

        // Cek apakah tim ini hasil custom (isRaw: false) dan punya data section
        // Sesuaikan .sections dengan nama variabel yang kamu pakai saat menyimpan data di halaman Create!
        if (!currentTeam.isRaw && currentTeam.customOriginalData && currentTeam.customOriginalData.sections) {
            teamSections = currentTeam.customOriginalData.sections; 
        }

        // Kosongkan container lalu render ulang
        if (sectionsContainer) {
            sectionsContainer.innerHTML = ''; 

            teamSections.forEach((sectionName, index) => {
                // Jadikan item pertama sebagai menu yang sedang "Aktif"
                if (index === 0) {
                    sectionsContainer.innerHTML += `
                        <li class="relative flex items-center h-14 bg-gray-800/40 cursor-pointer">
                            <div class="absolute left-0 top-0 bottom-0 w-1 bg-[#00ced1]"></div>
                            <span class="pl-8 font-bold text-white uppercase tracking-wider text-sm">${sectionName}</span>
                        </li>
                    `;
                } else {
                    // Menu lainnya
                    sectionsContainer.innerHTML += `
                        <li class="flex items-center h-14 hover:bg-gray-800/20 cursor-pointer transition">
                            <span class="pl-8 text-gray-400 uppercase tracking-wider text-sm">${sectionName}</span>
                        </li>
                    `;
                }
            });
        ...
        // Kalau user iseng buka URL tanpa ID atau tim sudah dihapus
        alert('Tim tidak ditemukan!');
        window.location.href = '/teams'; 
    }


    // --- 3. UI INTERAKSI DROPDOWN ---
    ...
    // --- 4. LOGIKA HAPUS PERMANEN DARI HALAMAN DETAIL ---
    function hapusTim() {
        ...
        if(confirmDelete) {
            ...
            if (currentTeam.isRaw) {
                // Hapus tim bawaan
                let deletedRawTeams = JSON.parse(localStorage.getItem('deletedRawTeams')) || [];
                if (!deletedRawTeams.includes(currentTeam.rawId)) {
                    deletedRawTeams.push(currentTeam.rawId);
                ...
                const indexToRemove = customTeamsFromStorage.findIndex(t => 
                    t.name === currentTeam.customOriginalData.name && 
                    t.description === currentTeam.customOriginalData.description
                );
            ...
            alert("Tim berhasil dihapus permanen!");
            // Auto redirect kembali ke halaman utama teams
            window.location.href = '/teams'; 
        }
        optionsDropdown.classList.add('hidden');
    }
