...
<body class="bg-[#f6f7fb] flex h-screen font-sans overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div class="flex flex-col items-start gap-4">
                <div class="w-32 h-32 bg-gray-600 rounded-2xl overflow-hidden shadow-lg border-2 border-gray-700 shrink-0">
                    <img src="<?= $team['profile_img'] ?>" alt="team logo" class="object-cover w-full h-full">
                </div>
    ...
    <div class="relative inline-block text-left">
        <button onclick="toggleTeamOptions(event)" class="text-gray-500 hover:text-white shrink-0 mt-1 focus:outline-none transition-colors">
            <i class="fa-solid fa-ellipsis-vertical text-lg"></i>
        </button>

        <div id="teamOptionsDropdown" class="hidden absolute right-0 mt-2 w-48 bg-[#2d3142] rounded-lg shadow-xl z-50 border border-gray-700 py-2 transform origin-top-right transition-all">
            ...
            <ul id="team-sections">
                <?php if (!empty($sections)): ?>
                    <?php foreach ($sections as $index => $sec): ?>
                        <?php 
                            $isActive = ($index === 0);
                            // Tambahkan class 'section-menu' sebagai penanda untuk JS
                            $baseClass = "section-menu pl-8 h-12 flex items-center border-l-4 transition-all duration-300 cursor-pointer group";
                            $activeClass = $isActive ? "bg-white/10 border-[#2dd4bf] text-[#2dd4bf]" : "border-transparent text-gray-300 hover:bg-white/5 hover:text-white";
                        ?>
                        <li class="<?= $baseClass ?> <?= $activeClass ?>" onclick="setActive(this)">
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <li class="pl-8 h-12 flex items-center text-gray-500 italic text-xs">Belum ada section</li>
                <?php endif; ?>
            </ul>
        </nav>
    </aside>
        ...
        <div class="flex-1 overflow-y-auto p-8 space-y-6">
            ...
            <?php if (!empty($problems)): ?>
                <?php foreach ($problems as $problem): ?>
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden max-w-4xl mx-auto transition-all duration-200 hover:scale-[1.03] hover:shadow-md cursor-pointer" onclick="window.location.href='/problem-detail?id=<?= $problem['id'] ?>'">
                        ...
                        <div class="p-6">
                            ...
                            <div class="text-gray-800 leading-relaxed mb-4 break-words whitespace-normal">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="text-center text-gray-500 py-10 font-medium">
                    Belum ada masalah yang diposting di tim ini.
                </div>
            <?php endif; ?>
    ...
    <div class="fixed bottom-8 right-8 w-16 h-16 bg-[#222831] rounded-2xl flex items-center justify-center cursor-pointer hover:scale-110 hover:border-2 hover:border-[#00ced1] hover:shadow-[0_6px_15px_rgba(0,173,181,0.25)] transition-all z-50" onclick="window.location.href='/Problemcreate?team_id=<?= $team['id'] ?>'">
        <div class="text-[#00ced1] border-[3px] border-[#00ced1] rounded-full flex items-center justify-center w-10 h-10">
            <iconify-icon icon="mdi:plus" style="font-size: 28px;"></iconify-icon>
        </div>
    </div>
